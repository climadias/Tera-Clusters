...
def carga_parlamentares():
    #########################################
    #    Carga de tabelas   #
    #########################################
    global df_parlamentares, df_parl_bens, df_deputados, df_gastos_gabinete , df_original
...
#Dados de municípios
def carga_municipios_votacao():
    #########################################
    #    Carga de tabelas   #
    #########################################
    global df_municipios_votacao, df_municipios
    con = sql.connect("../../dados/sql/base_completa.db")
...
def carga_municipios_geo():
    #########################################
    #    Carga de dataframe com shape   #
    #########################################
    global df_mapa_brasil
...
def ajusta_colunas_rafael():
    #########################################
    #    Ajusta dataframe não normalizado   #
    #########################################
...
def carga_atuacao_deputados():
    #########################################
    #    Carrega tabelas   #
    #########################################
    global df_dep_eventos, df_dep_eventos_presenca
    con = sql.connect("../../dados/sql/base_completa.db")
    df_camara=pd.read_sql("select * from deputados_cadastro",con)
    df_senado=pd.read_sql("select * from senadores_cadastro",con)
    df_dep_eventos=pd.read_sql("select * from eventos_info",con)
    df_dep_eventos_presenca=pd.read_sql("select * from eventos_presenca",con)
    con.close()
    return 
...
# Escolhe seleciona o parlamentar
def escolhe_cpf(df_parlamentares):
    #########################################
    #    Escolhe um CPF   #
    #########################################
    global cpf, df_cpf_votacao, df_cpf
    ...
    cpf=df_original["CPF"].sample(n=1).iloc[0]
    parlamentar=df_original[df_original["CPF"]==cpf]
    cargo=parlamentar["DS_CARGO"].iloc[0]
    #########################################
    #    Monta dataframe específico do parlamentar e de sua votação   #
    #########################################
...
def carga_informações():
    carga_parlamentares()
    carga_municipios_votacao()
    carga_municipios_geo()
    ajusta_colunas_rafael()
    # Carrega os dados de atuação parlamentar
    carga_atuacao_deputados()
    return

#########################################
#    exibe detalhes deputado#
#########################################
def detalhe_deputados(cpf):
    #escolhe_cpf(df_parlamentares)
    exibe_cadastro(cpf)
    exibe_genero(cpf)
    exibe_cor(cpf)
